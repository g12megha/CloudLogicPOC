...
![Application router overview diagram](app-router-diagram.png "Overview diagram")
...
During local development the configurations that the application router requires can be provided in files located in the working directory.
When the business application is deployed on XS Advanced, those configurations are taken from the corresponding environment variables:

Configuration | Local file | Corresponding environment variable | Optional | Description
------------- | ---------- | -------------------------- | :------: | -----------
[Main configuration](#xs-appjson-configuration-file) | xs-app.json | - | | The *xs-app.json* file contains the main configurations of the application router. This file is mandatory.
[Destinations configuration](#destinations) | local-destinations.json | destinations | x | Provides information about the available destinations.
[UAA configuration](#uaa-configuration) | default-services.json | VCAP_SERVICES | x | The *default-services.json* file and the corresponding `VCAP_SERVICES` environment variable provide configurations that represent XS services. The application router uses only the configuration of the User Account and Authentication (UAA).
[Additional headers configuration](#additional-headers-configuration) | local-http-headers.json | httpHeaders | x | Provides headers that the application router will return to the client in its responses.
[Plugins configuration](#plugins-configuration) | local-plugins.json | plugins | x | A plugin is just like a [*route*](#routes) except that you can't configure some inner properties.
[Session timeout](#session-timeout) | - | SESSION_TIMEOUT | x | Positive integer representing the session timeout in minutes. The default timeout is 15 minutes.
[X-Frame-Options](#x-frame-options-configuration) | - | SEND_XFRAMEOPTIONS, httpHeaders | x | Configuration for X-Frame-Options header value. |
[Whitelist service configuration](#whitelist-service) | - | CJ_PROTECT_WHITELIST | x | Configuration for the whitelist service preventing clickjack attacks.  |

Example content for every type of configuration can be found later in the document.


### Destinations

The destinations configuration is provided in the *local-destinations.json* file during local development
and in the `destinations` environment variable if the application is deployed on XS Advanced.
...
Additional headers can either be set in the `httpHeaders` environment variable or in the *local-http-headers.json* file.
...
A plugin serves almost the same purpose as [*routes*](#routes). The difference is that plugins can be configured through the environment and that way you can add new routes to the application router without changing the design-time artefact xs-app.json. The plugin configuration properties are the same as as those of a [*route*](#routes) except that you can't configure `localDir` and `replace`.

Property | Type | Optional | Description
-------- | ---- |:--------:| -----------
name | String | | The name of this plugin
source | String | | A string, representing the regular expression that matches an incoming request path.</br> **Note:** A request matches a particular route if its path __contains__ the given RegExp.</br> To ensure the RegExp matches the complete path, surround it with `^` and `$`.
...
After 40 minutes of user inactivity (no requests have been sent to the application router), a Central Logout will be triggered due to the session's timeout.
...
**Note:** You can also set this property as an environment varialbe.

### X-Frame-Options configuration

AppRouter sends `X-Frame-Options` header by default with value `SAMEORIGIN`. This behavior can be changed in 2 ways:
...
Property | Type | Optional | Description
-------- | ---- |:--------:| -----------
source | String | | A string representing the regular expression that matches the incoming request path.</br> **Note:** A request matches a particular route if its path __contains__ the given pattern.</br> To ensure the RegExp matches the complete path, surround it with `^` and `$`.
...
localDir | String | x | Folder in the [working directory](#working-directory) from which the application router will serve static content.
...
**Note:** The properties *`target`*, *`destination`* and *`localDir`* are optional, but at least one of them should be defined. <br />
**Note:** The properties *`destination`* and *`localDir`* cannot be used together in a single route.
...
For example, the source path is appended to the destination URL.
...
The supported tags for replacing enviroment variables are: `{{ENV_VAR}}` and `{{{ENV_VAR}}}`.
...
sessionTimeout | Number | x | This property is **deprecated**. To set the timeout use the [SESSION_TIMEOUT](#session-timeout) environment variable.
...
[destinations](#destinations-property) | Object | x | Additional options for your destinations (besides the ones in the *local-destinations.json*, the `destinations` environment variable or the *manifest.yml* file).
[compression](#compression-property) | Object | x | Configuration regarding compressing resources before responding to the client.
...
Holds the URL path to a web page that is served when the application router is called without a path. The html file is located in a folder with static resources.
Here we may distinguish two cases depending on the route configurations in the `routes` property of the *xs-app.json* configuration file:

- When none of the defined route configurations have a `localDir` property

If there are no routes with a `localDir`, then a default route is added **at the end** of the array with routes. The order of the routes is important.
This default route looks like:
...
In case the `welcomeFile` property looks like this:

```json
"welcomeFile": "hello-world.html"
```

The html file itself should be located at `resources/hello-world.html` under the working directory.

- When there is at least one route with a `localDir` property

In this case the `welcomeFile` should contain the URL path to the web page. Just like you would request it through the application router.
The application router will redirect to this URL when there is no path in the original incoming request.

If we have the following route defined in the `routes` array in our xs-app.json configuration file:
...
And have the `welcomeFile` property set like the following:

```json
"welcomeFile": "/web-pages/hello-world.html"
```

The *hello-world.html* should be located in `/my-static-resources/web-pages` under the working folder.
...
```
The value `node-backend`should be an object with the following properties:
...
In order to use the web socket communication, it should be enabled through the [approuter configuration](#top-level-properties).
...
**Note:** Non-session cookies from backend services are forwarded to the client. Cookie colisions may occur and the application should be able to handle them.

### Session Contents

* Location to redirect after login - in case the request is redirected to a UAA login form, the original request URL is stored in the session so after successful authentication, the user is redirected back to it.
...
The whitelist service reads allowed hostnames and domans from the environment variable `CJ_PROTECT_WHITELIST`.
The content is a JSON list of object with the following properties:
...
**Note**: Keep in mind that AppRouter sends by default the X-Frame-Options header with value `SAMEORIGIN`, in the case whitelist service is enabled, this header value probably needs to be changed, see the [X-Frame-Options]('#x-frame-options-configuration') header section for details how to change it.
...
The memory consumption of the application router is described in the [sizing guide](sizingGuide.md).
...
that the client will receive will be the one comming from the backend and not the one of the application router itself.
