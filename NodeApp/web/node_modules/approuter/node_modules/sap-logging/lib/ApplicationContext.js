...
function enableRuntimeLevelChanging(appContext) {
  ...
  if (RuntimeMessagingLib) {
    ...
    rtConn.on('ready', function () {
      ...
      consumer.on('handleDelivery', ApplicationContext.prototype.setLevel.bind(appContext));
...
ApplicationContext.prototype.setLevel = function (component, level) {
  this._levelsContainer.setLevel(component, level);
};
...
ApplicationContext.prototype.getTracer = function (id) {
  return this.createRequestContext(null, id || '').getTracer();
};
...
function attachOptions(appContext, options) {
  ...
  var opts = _.defaults({}, options, {
    logLocation: process.env.XS_APPLICATION_LOG_FILE || constants.STORAGE.CONSOLE,
    traceLocation: process.env.XS_TRACE_FILE || constants.STORAGE.CONSOLE
  });
...
function createTransport(location, options, fnFormatterFactory) {
  ...
  if (typeof location === 'string') {
    ...
    transport.formatter = fnFormatterFactory(false);
  }
  if (location === constants.STORAGE.CONSOLE) {
    transport = new winston.transports.Console(options);
    transport.formatter = fnFormatterFactory(true);
  }
  return transport;
}
