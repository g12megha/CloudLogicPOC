...
function Logger(reqContext, category) {
  ...
  if (appContext.logLocation !== appContext.traceLocation) {
    this._tracer = reqContext.getTracer();
  }
}

function resolveCategory(categoryArg) {
  assert(_.isString(categoryArg), 'Category argument should be a string');
  assert(/^\//.test(categoryArg), 'Category should start with a forward slash');
  return categoryArg;
}
...
function addFunctionForEveryLevel(proto, availableLevels) {
  Object.keys(availableLevels).forEach(function (levelName) {
    proto[levelName] = function () {
      ...
      if (this._tracer) {
        this._tracer._traceLogEntry(levelName, arrArgs, callStackElement);
      } else if (common.isError(arrArgs[0])) {
        // trace error stack, all the other arguments were already logged above
        this._reqContext.getTracer()._traceLogEntry(levelName, [arrArgs[0]], callStackElement);
      }
    };
  }, this);
}
...
function logEntry(logger, levelName, arrArgs) {
  ...
  var message = util.format.apply(null, arrArgs);
  var meta = {
    messageContext: {
